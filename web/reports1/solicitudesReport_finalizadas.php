...
if ($temp_ff ==0 and $temp_fi ==0)
    {$datos = $conexion->get_results("SELECT descripcion,fecha_requiere,fecha_recepcion FROM public.sifda_solicitud_servicio where id_estado = 4 "); }
else
    {$datos = $conexion->get_results("SELECT descripcion,fecha_requiere,fecha_recepcion FROM public.sifda_solicitud_servicio where where id_estado = 4 and fecha_recepcion between '$temp_fi' and '$temp_ff'"); }
